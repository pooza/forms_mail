...
class Spreadsheet_Excel_Writer_Parser extends PEAR
{
    ...
    */
    function _convertRange2d($range)
    {
        ...
        if (preg_match("/^([A-Ia-i]?[A-Za-z])(\d+)\:([A-Ia-i]?[A-Za-z])(\d+)$/", $range)) {
            list($cell1, $cell2) = mb_split(':', $range);
    ...
    */
    function _convertRange3d($token)
    {
        ...
        list($ext_ref, $range) = mb_split('!', $token);
    ...
    */
    function _convertRef3d($cell)
    {
        ...
        // Split the ref at the ! symbol
        list($ext_ref, $cell) = mb_split('!', $cell);
    ...
    */
    function _packExtRef($ext_ref)
    {
        ...
        if (preg_match("/:/", $ext_ref)) {
            list($sheet_name1, $sheet_name2) = mb_split(':', $ext_ref);
        ...
        switch($token) {
            ...
            default:
                ...
                if (preg_match('/^\$?[A-Ia-i]?[A-Za-z]\$?[0-9]+$/',$token) and
                   !mb_ereg("[0-9]",$this->_lookahead) and 
                ...
                elseif (mb_ereg("^\"[^\"]{0,255}\"$",$token))
                {
                    return $token;
                }
                // if it's a function call
                elseif (mb_eregi("^[A-Z0-9\xc0-\xdc\.]+$",$token) and ($this->_lookahead == "("))
                {
                    return $token;
                }
                return '';
    ...
    */
    function _expression()
    {
        ...
        if (mb_ereg("^\"[^\"]{0,255}\"$", $this->_current_token)) {
    ...
    */
    function _fact()
    {
        ...
        // if it's a function call
        elseif (mb_eregi("^[A-Z0-9\xc0-\xdc\.]+$",$this->_current_token))
        {
            $result = $this->_func();
            return $result;
        }
